...
{
    if(map.data.size() != map.info.width*map.info.height) return false;
    this->grid_.clear();
    ...
    this->origin_x_ = -1*(int)(map.info.origin.position.x);
    this->origin_y_ = -1*(int)(map.info.origin.position.y);

    int i=0;
    this->grid_.resize(this->height_ , std::vector<int>(this->width_, -1));
    for(const auto &cell: map.data) this->grid_[i/this->width_][(i++)%this->width_] = cell;
    // for(int y=0; y<this->height_; y++)
    // {
    //     this->grid_.push_back(std::vector<int>());
    //     for(int x=0; x<this->width_; x++)
    //     {
    //         this->grid_[y].push_back(map.data[i++]);
    //     }
    // }
    return true;
}
...
nav_msgs::OccupancyGrid Gridmap::set_grid()
{
    ...
    map.info.origin.position.x = this->origin_x_;
    map.info.origin.position.y = this->origin_y_;
